...
- Before any commit: `uv run ruff format .` → `uv run ruff check .` → `uv run pytest -q` — all clean, then re-stage. Expected baseline: **264 passed, 17 skipped, 1 xfailed** (342 on the GH200 with Triton).
